...
{
    /// <summary>
    /// Application for generating a <see cref="MacroManifest"/>.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The following options are used to generate a Product manifest:
    /// <code>
    /// Macro.Utilities.Manifest.ManifestGenerationApplication /d=C:\Projects\Macro\ImageServer\ShredHostService\bin\Debug\ /m=Manifest.xml ImageServerShredsInput.xml
    /// </code>
    /// </para>
    /// <para>
    /// The following options are used to generate a Package manifest:
    /// <code>
    /// Macro.Utilities.Manifest.ManifestGenerationApplication /d=C:\Projects\Macro\ImageServer\ShredHostService\bin\Debug\  /p+ /pn="J2K Package" /m=PackageManifest.xml /pm=Manifest.xml ImageServerShredsJ2kInput.xml
    /// </code>
    /// </para>
    /// <para>
    /// </para>
    ...
    public class ManifestGenerationApplication : IApplicationRoot
    {
        ...
        private readonly MacroManifest _manifest = new MacroManifest();
            ...
            foreach (ManifestInput input in inputs)
            {
                foreach(ManifestInput.InputFile inputFile in input.Files)
                {
                    ...
                    if (_manifest.ProductManifest != null)
                    {
                        ...
                        // common assembly
                        if (file.Filename.ToLower().Equals("common\\Macro.common.dll"))
        ...
        private void LoadConfiguration(string path)
        {
            ...
            XmlNodeList list = xmldoc.SelectNodes("configuration/applicationSettings/Macro.Common.ProductSettings/setting");
        ...
        {
            MacroManifest input = MacroManifest.Deserialize(_cmdLine.ProductManifest);
