...
namespace Macro.Dicom.Samples
{
    public partial class SamplesForm : Form
    {
		...
		private void ComboBoxQueryScuQueryTypeSelectedIndexChanged(object sender, EventArgs e)
		{
			...
			{
                Stream stream = GetType().Assembly.GetManifestResourceStream("Macro.Dicom.Samples.SampleXml.StudyRootStudy.xml");
				if (stream != null)
				{
					doc.Load(stream);
					stream.Close();
			...
			{
                var stream = GetType().Assembly.GetManifestResourceStream("Macro.Dicom.Samples.SampleXml.PatientRootPatient.xml");
				if (stream != null)
				{
					doc.Load(stream);
					stream.Close();
		...
		private void ComboBoxQueryScuQueryLevelSelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (comboBoxQueryScuQueryType.SelectedIndex == 0)
			{
				if (comboBoxQueryScuQueryLevel.SelectedItem.Equals("STUDY"))
                    xmlFile = "Macro.Dicom.Samples.SampleXml.StudyRootStudy.xml";
				else if (comboBoxQueryScuQueryLevel.SelectedItem.Equals("SERIES"))
                    xmlFile = "Macro.Dicom.Samples.SampleXml.StudyRootSeries.xml";
				else
                    xmlFile = "Macro.Dicom.Samples.SampleXml.StudyRootImage.xml";
			}
			else
			{
				if (comboBoxQueryScuQueryLevel.SelectedItem.Equals("PATIENT"))
                    xmlFile = "Macro.Dicom.Samples.SampleXml.PatientRootPatient.xml";
				else if (comboBoxQueryScuQueryLevel.SelectedItem.Equals("STUDY"))
                    xmlFile = "Macro.Dicom.Samples.SampleXml.PatientRootStudy.xml";
				else if (comboBoxQueryScuQueryLevel.SelectedItem.Equals("SERIES"))
                    xmlFile = "Macro.Dicom.Samples.SampleXml.PatientRootSeries.xml";
				else
                    xmlFile = "Macro.Dicom.Samples.SampleXml.PatientRootImage.xml";
			}
        ...
        private void ComboBoxMoveScuQueryTypeSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboBoxMoveScuQueryType.SelectedIndex == 0)
            {
                Stream stream = GetType().Assembly.GetManifestResourceStream("Macro.Dicom.Samples.SampleXml.StudyRootMoveStudy.xml");
                if (stream != null)
                {
                    doc.Load(stream);
                    stream.Close();
            ...
            {
                var stream = GetType().Assembly.GetManifestResourceStream("Macro.Dicom.Samples.SampleXml.PatientRootMovePatient.xml");
                if (stream != null)
                {
                    doc.Load(stream);
                    stream.Close();
