...
namespace Macro.Common
{
	...
	public static class Platform
	{
        ...
        private static string GetApplicationDataDirectory()
        {
            ...
            var path = Platform.InstallDirectory;
            path = Path.Combine(path, "Macro_Inc"); //TODO this seems to be derived from the AssemblyCompanyAttribute
            if (string.IsNullOrEmpty(ProductInformation.FamilyName))
            {
                string name = ProductInformation.GetName(true, false);
                path = Path.Combine(path, name);
            }
              
            else
            {
               string name= string.Format("{0} {1}", ProductInformation.Component, ProductInformation.FamilyName);
               path = Path.Combine(path, name);
            }
