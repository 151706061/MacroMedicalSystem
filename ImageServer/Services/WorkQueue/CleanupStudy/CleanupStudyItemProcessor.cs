...
using Macro.ImageServer.Core.Command;
using Macro.Dicom.Utilities.Command;
using System.Collections.Generic;
using System.Text;
using System.Diagnostics;

namespace Macro.ImageServer.Services.WorkQueue.CleanupStudy
{
    /// <summary>
            ...
            foreach (WorkQueueUid sop in WorkQueueUidList)
            {
                ...
                using (ServerCommandProcessor processor = new ServerCommandProcessor(String.Format("Deleting {0}", sop.SopInstanceUid)))
                {
                    ...
                    try
                    {
                        ...
                        Macro.ImageServer.Core.Command.DeleteDirectoryCommand deleteDir = new Macro.ImageServer.Core.Command.DeleteDirectoryCommand(fileInfo.Directory.FullName, false, true);
                ...
                if (Directory.Exists(baseDir))
                {
                    using (ServerCommandProcessor processor = new ServerCommandProcessor(String.Format("Deleting {0}", sop.SopInstanceUid)))
                    {
                        Macro.ImageServer.Core.Command.DeleteDirectoryCommand deleteDir = new Macro.ImageServer.Core.Command.DeleteDirectoryCommand(baseDir, false, true);
                        processor.AddCommand(deleteDir);
