		...
		private class DefaultContextMenuActionFactory : ActionFactory
		{
			...
			private string GetActionMessage(IDicomImageSetDescriptor descriptor)
			{
				if (descriptor.IsOffline)
                    return Macro.ImageViewer.SR.MessageInfoStudyOffline;
				else if (descriptor.IsNearline)
                    return Macro.ImageViewer.SR.MessageInfoStudyNearline;
				else if (descriptor.IsInUse)
                    return Macro.ImageViewer.SR.MessageInfoStudyInUse;
				else if (descriptor.IsNotLoadable)
                    return Macro.ImageViewer.SR.MessageInfoNoStudyLoader;
				else
                    return Macro.ImageViewer.SR.MessageInfoStudyCouldNotBeLoaded;
			}
